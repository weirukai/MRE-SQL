...
def get_llm_chain(engine_name: str, temperature: float = 0, sampling_count=1, base_url: str = None) -> Any:
    """
    ...
    if sampling_count:
        # sampling multiple CoT and answers with high temperature
        params['n'] = sampling_count

    # Adjust base_url if provided
    if base_url and "openai_api_base" in params:
        params["openai_api_base"] = f"{base_url}/v1"
    ## top_p:
    params['top_p'] = 0.9
...
def call_llm_chain_multi_sampling(prompt: Any, engine: Any, parser: Any, request_kwargs: Dict[str, Any], step: int,
                                  max_attempts: int = 12, backoff_base: int = 2, jitter_max: int = 60) -> Any:
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            llm_outputs = engine.generate([prompt_text])


            outputs = []
            for output_item in llm_outputs.generations[0]:
                content = output_item.message.content
...
def call_llm_chain(prompt: Any, engine: Any, parser: Any, request_kwargs: Dict[str, Any], step: int,
                   max_attempts: int = 12, backoff_base: int = 2, jitter_max: int = 60) -> Any:
    """
    Calls the LLM chain with exponential backoff and jitter on failure.
