...
from src.database_utils.spider_exec_eval import result_eq, eval_exec_match
...
def create_smaller_db(original_db_path, max_rows=100000):
    ...
    for table in tables:
        ...
        cursor_new.executemany(f"INSERT INTO `{table_name}` VALUES ({','.join(['?' for _ in range(len(rows[0]))])})",
                               rows)
        conn_new.commit()
...
def _compare_sqls_outcomes(db_path: str, predicted_sql: str, ground_truth_sql: str) -> int:
    ...
    if "spider" in str(db_path):
        try:
            result = eval_exec_match(str(db_path), p_str=predicted_sql, g_str=ground_truth_sql, plug_value=False,
                                     keep_distinct=True, progress_bar_for_each_datapoint=False)
            if result != 1:
                return predicted_res == ground_truth_res
            return result
            # order_matters = "orderby" in re.sub(r"\s+", ground_truth_sql.lower(), "")
...
def compare_sqls(db_path: str, predicted_sql: str, ground_truth_sql: str, meta_time_out: int = 90) -> Dict[
    str, Union[int, str]]:
    """
    ...
    try:
        ...
        logging.warning(f"Error in compare_sqls: {e}")
        error = str(e)
        res = 0
