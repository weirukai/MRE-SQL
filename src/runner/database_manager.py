...
from database_utils.execution import execute_sql, compare_sqls, validate_sql_query, aggregate_sqls, \
    get_execution_status, subprocess_sql_executor
...
class DatabaseManager:
    ...
    def _set_paths(self):
        ...
        self.db_path = DB_ROOT_PATH/f"{self.db_mode}" / f"{self.db_mode}_databases" / self.db_id / f"{self.db_id}.sqlite"
        self.db_directory_path = DB_ROOT_PATH/f"{self.db_mode}" / f"{self.db_mode}_databases" / self.db_id
    ...
    def query_lsh(self, keyword: str, signature_size: int = 100, n_gram: int = 3, top_n: int = 10) -> Dict[
        str, List[str]]:
        """
