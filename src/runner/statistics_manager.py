...
class Statistics:
    ...
    def from_dict(self, data: dict) -> None:
        ...
        self.errors = {}
        self.total = {}
        for key, counts in data.get("counts", {}).items():
            self.total[key] = counts["total"]

        for key, entries in data.get("ids", {}).items():

            correct_list = entries.get("correct", [])
            if correct_list:
                self.corrects[key] = [tuple(item) for item in correct_list]

            incorrect_list = entries.get("incorrect", [])
            if incorrect_list:
                self.incorrects[key] = [tuple(item) for item in incorrect_list]

            error_list = entries.get("error", [])
            if error_list:
                parsed_errors = []
                for item in error_list:
                    if len(item) in (2, 3):
                        parsed_errors.append(tuple(item))
                    else:
                        raise ValueError(f"Invalid error tuple length: {len(item)} in {item}")
                self.errors[key] = parsed_errors
...
class StatisticsManager:
    def __init__(self, result_directory: str):
        ...
        if not self.statistics_file_path.exists():
            ...
            with open(self.statistics_file_path, 'r', encoding='utf8') as f:
                statistics_json = json.load(f)
                self.statistics.from_dict(statistics_json)
