...
class RunManager:
    ...
    def __init__(self, args: Any):
        ...
        if self.args.resume_path:
            ...
            result_directory = Path(self.RESULT_ROOT_PATH) / self.args.data_mode / self.args.config[
                "setting_name"] / Path(self.args.data_path).stem / self.args.resume_path
            if not os.path.exists(result_directory):
                raise "Resume Path {} Not Found!".format(self.args.resume_path)
            self.result_directory = result_directory
            self.statistics_manager = StatisticsManager(self.result_directory)
            self.tasks: List[Task] = []
            self.total_number_of_tasks = 0
            self.processed_tasks = list(self.statistics_manager.statistics.total.values())[0]
            print("Resume from path {}".format(result_directory))
        else:
    ...
    def get_result_directory(self, resume_path=None) -> str:
        """
    ...
    def initialize_tasks(self, dataset: List[Dict[str, Any]]):
        ...
        """
        for i, data in tqdm.tqdm(enumerate(dataset), total=len(dataset), desc="Loading Tasks:"):
            if "question_id" not in data:
                data = {"question_id": i, **data}
            if self.args.resume_path and i < self.processed_tasks:
                pass
            else:
    ...
    def run_tasks(self):
        ...
        if self.args.num_workers > 1:
            ...
            for i, task in enumerate(self.tasks):
                ...
                self.task_done(log)
        print(f"Tasks completed, with {len(self.tasks)} tasks in total")
