...
class Agent:
    ...
    def __init__(self, name: str, task: str, config: dict):
        ...
        self.tools = {}
        self.chat_history = []
        self.current_tool_index = 0  # Track the current tool index
    ...
    def workout(self, system_state: SystemState) -> SystemState:
        ...
        try:
            for i in range(10):
                # response = self.call_llm_agent(system_state)
                response = self.call_rule_agent(system_state)
                formatted_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                print(f"{formatted_time} - INFO - Agent {self.name} response: {response}")
                if self.is_done(response):
                    break
                tool_name = self.get_next_tool_name_json(response)
    ...
    def get_next_tool_name_json(self, response: str) -> str:
        try:
    ...
    def call_llm_agent(self, system_state: SystemState) -> SystemState:
        """
        Call the llm agent with the given system state.
        This is the planning agent that helps to select the tools.
        """
    ...
    def call_rule_agent(self, system_state: SystemState) -> SystemState:
        """
              Call the rule agent to return the next tool in order.
              If all tools have been returned, return "DONE".
              """

        if self.current_tool_index < len(self.tools):
            # Get the current tool name
            tool_name = list(self.tools.keys())[self.current_tool_index]
            # Increment the index for the next call
            self.current_tool_index += 1
            # Update the system state with the tool name
            return "{{\"tool\":\"{}\",\"reason\":\"Call the tool {}\"}}".format(tool_name, tool_name)
        else:
            # All tools have been returned, set the state to "DONE"
            return "{\"tool\":\"DONE\",\"reason\":\"Task is finished!\"}"
