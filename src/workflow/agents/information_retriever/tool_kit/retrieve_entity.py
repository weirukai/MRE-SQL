...
from src.workflow.agents.information_retriever.tool_kit.LocalEmbedding import LocalEmbedding
...
class RetrieveEntity(Tool):
    ...
    def __init__(self, embedding_function):
        ...
        self.embedding_function = LocalEmbedding(embedding_function)
    ...
    def _run(self, state: SystemState):
        ...
        state.similar_columns = self._get_similar_columns(keywords=state.keywords, question=state.task.question,
    ...
    def _get_similar_entities(self, keywords: List[str]) -> Dict[str, Dict[str, List[str]]]:
        ...
        for table_name, column_values in selected_values.items():
            for column_name, values in column_values.items():
                ...
                values = [entity for entity in values if
                          entity["edit_distance_similarity"] >= 0.9 * max_edit_distance_similarity]
                max_embedding_similarity = max(entity["embedding_similarity"] for entity in values)
                selected_values[table_name][column_name] = [entity['similar_value'] for entity in values if entity[
                    "embedding_similarity"] >= 0.9 * max_embedding_similarity]
    ...
    def _get_similar_entities_via_edit_distance(self, similar_entities_via_LSH: List[Dict[str, Any]]) -> List[
        Dict[str, Any]]:
        similar_entities_via_edit_distance_similarity = []
        for entity_packet in similar_entities_via_LSH:
            edit_distance_similarity = difflib.SequenceMatcher(None, entity_packet["substring"].lower(),
                                                               entity_packet["similar_value"].lower()).ratio()
    ...
    def _get_similar_entities_via_embedding(self, similar_entities_via_edit_distance: List[Dict[str, Any]]) -> List[
        Dict[str, Any]]:
        similar_values_dict = {}
        to_embed_strings = []
