...
class Evaluate(Tool):
    ...
    def _run(self, state: SystemState):
        ...
        try:
            ...
            response = [r[0] for r in response if ["OutputParserException"] != r]
        ...
        for item in response:
            ...
            if item is not None and len(item['scores']) == len(target_SQL_meta_infos):
        ...
        #     comparison_matrix = []
        #     for item in response:
        #         if len(item['scores']) == len(target_SQL_meta_infos):
        #             comparison_matrix.append(item['scores'])
        #     self.comparison_matrix = comparison_matrix
        #     scores = [sum([score[index] for score in comparison_matrix]) for index in
        #               range(len(comparison_matrix[0]))]
        self.scores = scores
    ...
    def pick_the_best_candidate(self, scores: List[int], candidates: List[SQLMetaInfo],
                                candidate_clusters: Dict) -> SQLMetaInfo:
        """
